...
`credentialsFile` names a gitignored file at your repo root:
...
Copy it from `runtime/credentials.example.js`. The runtime scripts read it;
**the agent never opens it and never prints its values**. A credential that
reaches a report, a log, or a tracker comment is a defect in this tool, not a
