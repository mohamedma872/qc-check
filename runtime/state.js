...
//
// State lives in <reportsDir>/<TICKET>-state.json (reportsDir comes from
// project.reportsDir in qc.config.json).
//
// Usage:
...
//   node runtime/state.js <TICKET> reset
//
// Everything a resumed run needs lives on disk: this state file (phases,
// sub-steps, notes, findings), the approved plan <reportsDir>/<TICKET>-plan.md,
// and the evidence files in <reportsDir>. A re-run after ANY interruption
// resumes from here - it never starts from scratch.
//
...
// pass (the user approved the DoD + test cases in <TICKET>-plan.md) or
...
if (ticketArg === '--help' || ticketArg === '-h' || cmd === '--help') {
  ...
  console.log('State file: <project.reportsDir>/<TICKET>-state.json (qc.config.json).');
  process.exit(0);
}
if (!ticketArg || !cmd) {
  usage();
...
const { reportsDir, validateTicket } = require('./config.js');
...
const file = path.join(dir, `${ticket}-state.json`);
...
function requirePlanApproved(state, phase, status) {
  ...
  console.error(
    ...
    `   Write ${path.join(relDir, `${ticket}-plan.md`)}, get approval, then: state.js ${ticket} set test-plan pass\n` +
...
switch (cmd) {
  ...
  case 'set': {
    ...
    const state = load() || { ticket, phases: {} };
