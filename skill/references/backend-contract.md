...
| Credentials | `config.credentialsFile` (gitignored, never printed) |
...
flavor (`QC_FLAVOR`, else `config.app.defaultFlavor`) decides which package the
device runs (`config.app.flavors[<flavor>].androidPackage` /
`.iosBundleId`), and for most apps the environment is baked in at build time.
If they disagree, the app is talking to a different server than you are and
every result is meaningless.
...
3. **Cross-check the LIVE deployed gateway.** Confirm the route is deployed on
   ...
   node qc/api.js --unauth GET '/v1/<path>'          # 401/403 = deployed, auth wall
   # Authenticated read of the endpoint the screen uses:
   node qc/api.js GET '/v1/<path>'
   # Write route existence, EMPTY body only (validation must reject it):
   node qc/api.js POST '/v1/<path>' --data '{}'      # 400/422 = deployed, validation ran
   ```
...
The approved test plan (`<reportsDir>/ABC-123-plan.md`) must already have an
...
2. **Auth:**
   ...
   credentials from `config.credentialsFile`. Never print the credential values,
   and never paste a full token into a report or a ticket.
...
3. **Feature round-trip ON THE SERVER.** Prove the backend half without the app,
   ...
   - **Write feature** - perform the write through `qc/api.js` **on the QC
     ...
     up. Do not discover it on-device.
